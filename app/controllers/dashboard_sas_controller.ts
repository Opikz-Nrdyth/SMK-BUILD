...
export default class DashboardSasController {
  public async SuperAdmin({ inertia }: HttpContext) {
    try {
      let stats = {
        totalSiswa: 0,
        totalGuru: 0,
      ...
      const [totalSiswa, totalGuru, totalKelas, totalMapel, totalWali, totalStaf] =
        await Promise.all([
      ...
      const tigaPuluhHariTerakhir = Array.from({ length: 30 }, (_, i) =>
        DateTime.now()
          .minus({ days: 29 - i })
          .toISODate()
      )

      const chartAbsensiData = await Promise.all(
        tigaPuluhHariTerakhir.map(async (date) => {
          if (!date) return { date: '', total: 0 }
      ...
      const pembayaranByJenis: Record<string, number> = {}
      ...
      // Data untuk donut chart status absensi
      ...
      const trendSiswa =
        totalSiswaBulanLalu > 0
          ? ((((totalSiswaBulanIni - totalSiswaBulanLalu) / totalSiswaBulanLalu) * 100).toFixed(
              1
            ) as string)
      ...
      const getTrendText = (trend: string, type: string) => {
        ...
        if (type === 'absensi') {
          if (trendNum > 0) return `dari kemarin`
          if (trendNum < 0) return `dari kemarin`
          return 'Sama dengan kemarin'
        }

        if (trendNum > 0) return `dari bulan lalu`
        if (trendNum < 0) return `dari bulan lalu`
  ...
  public async Guru({ inertia, auth }: HttpContext) {
    try {
      await auth.check()
      ...
      // Map: nisn -> namaKelas
      const nisnDiampu = new Map<string, string>()
      const kelasMap = new Map<string, { namaKelas: string; jumlahSiswa: number }>()
      ...
      // Data absensi hari ini untuk siswa yang diampu
      const absensiHariIni = await DataAbsensi.query()
        .where('hari', 'LIKE', `${DateTime.now().toISODate()}%`)
      ...
      // Data absensi 7 hari terakhir untuk chart
      ...
      const chartAbsensiData = await Promise.all(
        tujuhHariTerakhir.map(async (date) => {
          if (!date) return { date: '', total: 0 }
      ...
      // Data absensi bulan ini berdasarkan status
      const startOfMonth = DateTime.now().startOf('month')
      const endOfMonth = DateTime.now().endOf('month')
  ...
  public async Staf({ inertia, auth }: HttpContext) {
    try {
      await auth.check()
      const user = auth.user!
      ...
      // Data berbeda berdasarkan departemen
      switch (departemen) {
        case 'Administrasi':
  ...
  public async Siswa({ inertia, request, auth }: HttpContext) {
    try {
      await auth.check()
      const user = auth.user!
      ...
      const persentaseKehadiran =
        totalAbsensi > 0 ? Math.round((absensiHadir / totalAbsensi) * 100) : 0

      // Process statistik ujian
      ...
      // Ujian yang akan datang (3 hari ke depan)
      const ujianAkanDatang = ujianTersedia.filter((ujian) => {
        const tanggalUjian =
          typeof ujian.tanggalUjian == 'string'
   ...
   */
  private async getAbsensiChartData(userId: string, month?: string): Promise<any> {
    // Default ke bulan sekarang jika tidak ada parameter
    const currentMonth = month ? DateTime.fromISO(month) : DateTime.now()
    const startOfMonth = currentMonth.startOf('month')
    const endOfMonth = currentMonth.endOf('month')

    // Ambil data absensi bulan yang dipilih
    const absensiData = await DataAbsensi.query()
      .where('user_id', userId)
      .whereBetween('hari', [startOfMonth.toSQLDate() ?? '', endOfMonth.toSQLDate() ?? ''])
      .orderBy('hari', 'asc')

    // Buat calendar grid untuk bulan ini
    const calendarData = []

    // Tambahkan hari dari minggu sebelumnya jika perlu
    ...
    while (currentDate <= endDate) {
      ...
      const isSunday = currentDate.weekday === 1 // 7 = Minggu
      ...
      calendarData.push({
        ...
        status: absensiHariIni
          ...
          : isCurrentMonth
            ? 'Tidak Ada Data'
            : 'Bulan Lain',
        warna: this.getStatusColor(
          absensiHariIni ? absensiHariIni.status : isCurrentMonth ? 'Tidak Ada Data' : 'Bulan Lain'
        ),
      })
  ...
  /**
   * Helper function untuk mendapatkan status ujian
   */
  private getUjianStatus(kehadiran: ManajemenKehadiran): string {
  ...
  // Method untuk Staf Administrasi
  private async getAdminData(): Promise<[any, any, any]> {
    const [
      totalSiswa,
      totalGuru,
      totalKelas,
  ...
  // Method untuk Staf Keuangan
  private async getFinanceData(): Promise<[any, any, any]> {
    const semuaPembayaran = await DataPembayaran.all()

    // Analisis data pembayaran
    const pembayaranByJenis: Record<string, { total: number; count: number }> = {}
    ...
    semuaPembayaran.forEach((pembayaran) => {
      ...
      totalPendapatanBulanIni += pembayaranBulanIni.reduce(
  ...
  // Method untuk Staf Multimedia
  private async getMultimediaData(): Promise<[any, any, any]> {
    const [totalBlog, totalAktivitas, totalAds, blogPublished, aktivitasPublished, adsAktif] =
      await Promise.all([
  ...
  // Method untuk trend pendapatan bulanan (Keuangan)
  private async getMonthlyPendapatanTrend(): Promise<any[]> {
  ...
  // Method default untuk staf tanpa departemen spesifik
  private async getDefaultStaffData(): Promise<[any, any, any]> {
    return [
      {
