  ...
  private registerHandlers() {
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('send_message', (data) => {
        ...
        // Broadcast ke semua dalam room (termasuk pengirim)
      ...
      socket.on('whatsapp_get_qr', () => {
        try {
          ...
          if (qrCode) {
            ...
            socket.emit(
              'whatsapp_status',
              status.isReady ? 'WhatsApp sudah terhubung' : 'QR code belum tersedia'
            )
          }
        } catch (error: any) {
          socket.emit('whatsapp_error', error.message)
