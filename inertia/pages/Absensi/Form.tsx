...
import { AbsensiForm, AbsensiItem, AbsensiRecord, AbsensiSiswa } from './types'
...
interface Props {
  initialValues?: AbsensiItem
  onSubmit: (data: AbsensiItem) => void
  submitLabel: string
  dark?: boolean
  userOptions?: options[]
...
export default function Form({
  ...
  const { props } = usePage()
  const pattern = props?.pattern.split('/').filter((item) => item != '')

  const [siswa, setSiswa] = useState<AbsensiSiswa[]>([])
  ...
  const loadSiswaByKelas = async (kelasId: string) => {
    try {
      const response = await fetch(`siswa/${kelasId}`)
  ...
  const handleStatusChange = (userId: string, status: AbsensiSiswa['status']) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validasi data
    if (!data.kelasId || !data.mapelId || data.absensi?.length === 0) {
      alert('Harap lengkapi semua field dan pastikan ada data siswa')
      return
    }
    ...
    try {
      if (!isOnline && isStorageSupported) {
        ...
        const pendingData = await offlineStorage.getPendingAbsensi()
        const absensiRecords: AbsensiRecord[] = data.absensi.map((item: any) => ({
          userId: item.userId,
          ...
          (newRecord) =>
            !pendingData.some(
              (pending) =>
                ...
                pending.mapelId === newRecord.mapelId &&
        ...
        // Submit langsung ke server dengan header online status
        router.post(`/${pattern[0]}/${pattern[1]}`, data)
      }
    } catch (error: any) {
  ...
  const syncPendingData = async () => {
    try {
      ...
      if (pendingData?.length > 0) {
        router.post(`${pattern[0]}/${pattern[1]}/bulk`, payloadData, {
          onSuccess: async () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <span className="font-medium dark:text-white">Pesan: </span>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          ...
          <UniversalInput
            ...
            value={data.mapelId}
            onChange={(v: any) => setData('mapelId', v)}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y ">
                  {siswa.map((siswaItem, index) => (
                    <tr key={siswaItem.userId} className="hover:bg-gray-50 hover:bg-gray-700">
        ...
        <div className="flex justify-end pt-6 border-t border-gray-200">
          <button
            type="submit"
            disabled={processing || !data.kelasId || !data.mapelId || data.absensi?.length === 0}
